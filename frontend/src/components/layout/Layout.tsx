...
export function Layout() {
  ...
  useEffect(() => {
    ...
    const instructorAdminPaths = ['/approvals', '/courses', '/strategic'];
  ...
  useEffect(() => {
    ...
    const handleNotification = (data: INotification) => {
      ...
      setNotifications((prev) => [
        { ...data, id: Date.now(), is_read: false, created_at: new Date() },
        ...prev,
      ]);
  ...
  const handleAIAction = async (action: string, payload?: unknown) => {
    try {
      const data = payload as { id?: number } | undefined;
      if (action === 'approve-all') {
        ...
        if (data?.id) {
          await apiClient.patch(`/api/bookings/${data.id}`, { status: 'APPROVED' });
          toast.success(`${t('approve_booking_success')}${data.id}`);
        }
      } else if (action === 'cancel-booking') {
        if (data?.id) {
          // Dùng PATCH thay vì PUT theo đúng chuẩn Controller Backend hiện tại
          await apiClient.patch(`/api/bookings/${data.id}`, { status: 'CANCELED' });
          toast.success(`${t('cancel_booking_success')}${data.id}`);
        }
      }
  ...
  return (
    <div className="flex h-screen w-full bg-[#F8FAFC] dark:bg-slate-950 text-[#0F172A] dark:text-slate-200 font-sans overflow-hidden transition-colors duration-300">
      {/* Sidebar (240px or 80px) */}
      <aside className={clsx("bg-white dark:bg-slate-900 border-r border-[#E2E8F0] dark:border-slate-800 flex flex-col flex-shrink-0 z-20 shadow-[4px_0_24px_rgba(0,0,0,0.02)] transition-all duration-300", isSidebarCollapsed ? "w-[80px]" : "w-[240px]")}>
        {/* Logo */}
        <div className={clsx("h-[64px] flex items-center border-b border-[#E2E8F0] dark:border-slate-800", isSidebarCollapsed ? "justify-center" : "gap-3 px-6")}>
          <img
            src="/Logo-Dai-Hoc-Viet-Nhat-VJU.png"
            alt="VJU"
            className="w-8 h-8 object-contain rounded flex-shrink-0"
          />
          {!isSidebarCollapsed && (
            <div className="font-bold text-xl bg-gradient-to-r from-[#1E40AF] to-[#3B82F6] bg-clip-text text-transparent whitespace-nowrap overflow-hidden">
              LabBook
            </div>
          )}
        </div>
        ...
        {/* Navigation */}
        <nav className={clsx("flex-1 py-6 space-y-1 overflow-y-auto", isSidebarCollapsed ? "px-2" : "px-4")}>
          <div 
            className={clsx("flex items-center mb-4 cursor-pointer group", isSidebarCollapsed ? "justify-center" : "justify-between px-2")}
            onClick={() => setIsSidebarCollapsed(!isSidebarCollapsed)}
            title={isSidebarCollapsed ? "Mở rộng menu" : "Thu gọn menu"}
          >
            {!isSidebarCollapsed && (
              <div className="text-[12px] font-semibold text-[#757575] dark:text-slate-400 uppercase tracking-wider">
                {t('main_menu')}
              </div>
            )}
            <Menu className="w-5 h-5 text-[#757575] dark:text-slate-400 hover:text-blue-600 transition-colors" />
          </div>
        ...
        {/* User Profile Bottom */}
        <div className="p-4 border-t border-[#E0E0E0] dark:border-slate-800 flex flex-col items-center">
          <div
            onClick={() => navigate('/settings')}
            className={clsx("flex items-center gap-3 mb-4 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 rounded-xl transition-colors group w-full", isSidebarCollapsed ? "justify-center p-0" : "p-2 -mx-2")}
          >
            <div className="w-10 h-10 flex-shrink-0 rounded-full bg-[#D6E4F7] dark:bg-blue-900/30 text-[#1E5FA5] dark:text-blue-400 flex items-center justify-center font-bold overflow-hidden shadow-inner group-hover:ring-2 ring-blue-500/50 transition-all">
              {user?.avatar_url ? (
            ...
            {!isSidebarCollapsed && (
              <div className="flex-1 min-w-0">
                <div className="text-[14px] font-semibold text-[#212121] dark:text-slate-200 truncate group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                  {user
                    ? user.name
                    : isAdmin
                      ? 'Admin System'
          ...
          <button
            ...
            className={clsx("flex items-center text-[#C62828] hover:bg-[#F5F5F5] dark:hover:bg-slate-800 dark:bg-slate-800/50 rounded-md transition-colors font-medium", isSidebarCollapsed ? "justify-center w-10 h-10 p-0" : "w-full gap-2 px-2 py-2 text-[14px]")}
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[#F8FAFC] dark:bg-slate-950 transition-colors duration-300">
        ...
        <header className="h-[64px] bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-[#E2E8F0] dark:border-slate-800 flex items-center justify-between px-6 flex-shrink-0 z-40 sticky top-0 transition-colors duration-300">
          ...
          <div className="flex items-center text-[14px] font-medium text-[#64748B] dark:text-slate-400">
            {t('home')} <span className="mx-2 text-[#CBD5E1]">/</span>{' '}
            <span className="text-[#0F172A] dark:text-slate-100 font-semibold">{pageTitle}</span>
          </div>
            ...
            <div className="relative">
              ...
              {showSearchDropdown && (
                <div className="absolute top-full mt-2 w-full bg-white dark:bg-slate-900 border border-[#E2E8F0] dark:border-slate-800 rounded-xl shadow-lg dark:shadow-slate-900/50 z-50 overflow-hidden">
                  {isSearching ? (
                    <div className="p-4 text-center text-sm text-[#64748B] dark:text-slate-400">
                      {t('searching')}
                    </div>
                        ...
                        >
                          <div
                            className={clsx(
                              ...
                              item.type === 'ROOM'
                                ? 'bg-[#EFF6FF] text-[#3B82F6] dark:text-blue-400'
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="text-[13px] font-semibold text-[#0F172A] dark:text-slate-100 truncate">
                              {item.name}
                            </div>
                            <div className="text-[12px] text-[#64748B] dark:text-slate-400 truncate">
                              {item.description}
                            </div>
                          </div>
                        </div>
            ...
            <div className="relative" ref={langRef}>
              ...
              >
                ...
                <span className="text-[13px] font-semibold">
                  {LANGUAGES.find((l) => l.code === (i18n.language || 'vi'))?.label || 'VI'}
                </span>
                ...
                {showNotifDropdown && (
                  ...
                  >
                    <div className="p-3 border-b border-[#E2E8F0] dark:border-slate-800 bg-[#F8FAFC] dark:bg-slate-800/20 flex justify-between items-center">
                      <span className="font-bold text-[#0F172A] dark:text-slate-100 text-[14px]">
                        {t('notifications')}
                      </span>
                      <button
                        onClick={() => {
                    ...
                    <div className="max-h-80 overflow-y-auto">
                      {notifications.length === 0 ? (
                        <div className="p-6 text-center text-[13px] text-[#64748B] dark:text-slate-400">
                        ...
                        notifications.map((n, idx) => (
                          ...
                          >
                            <div className="font-semibold text-[#0F172A] dark:text-slate-100 text-[13px]">
                              {n.title}
                            </div>
                            <div className="text-[#475569] dark:text-slate-400 text-[12px] mt-0.5 line-clamp-2">
                              {n.message}
                            </div>
                          </div>
                        ))
...
function NavItem({ to, icon, label, isSidebarCollapsed }: { to: string; icon: React.ReactNode; label: string; isSidebarCollapsed?: boolean }) {
  const location = useLocation();
  ...
  return (
    <NavLink
      ...
      className={clsx(
        'flex items-center gap-3 py-2.5 rounded-xl transition-all duration-300 text-[14px] font-medium group relative overflow-hidden',
    ...
    >
      ...
      <div
        className={clsx(
          'w-5 h-5 flex items-center justify-center transition-transform duration-300 relative z-10 flex-shrink-0',
          {
            'scale-110 text-blue-600 dark:text-blue-400': isActive,
      ...
      {!isSidebarCollapsed && (
        <span className="relative z-10 whitespace-nowrap overflow-hidden transition-all duration-300">{label}</span>
      )}
    </NavLink>
  );
}
