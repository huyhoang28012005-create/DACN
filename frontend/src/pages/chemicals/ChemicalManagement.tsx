...
import { Plus, Search, Edit2, Trash2, Beaker, ClipboardList, X, AlertTriangle } from 'lucide-react';
import { chemicalService } from '../../services';
import { ConfirmModal } from '../../components/common/ConfirmModal';
import { StatMini } from '../../components/ui/StatMini';
import { IChemical } from '../../types/models';
...
export function ChemicalManagement() {
  ...
  const fetchChemicals = async () => {
    ...
    try {
      ...
      const msg = err.response?.data?.message || t('load_chemicals_error');
  ...
  const handleOpenModal = (chemical?: IChemical) => {
    if (chemical) {
      setFormData({
        id: chemical.id,
        name: chemical.name,
        ...
        expiration_date: chemical.expiration_date
          ...
          : '',
        storage_conditions: (chemical as any).storage_conditions || '',
        min_stock_alert: (chemical as any).min_stock_alert || 5,
      });
      setIsEditing(true);
    } else {
      setFormData({
  ...
  const handleOpenUsageModal = (chemical: IChemical) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload = {
        ...
        min_stock_alert: Number((formData as any).min_stock_alert || 5),
      ...
      const msg =
        ...
        (isEditing ? t('update_chemical_failed') : t('add_chemical_failed'));
      toast.error(Array.isArray(msg) ? msg[0] : msg);
    }
  };
  ...
  const handleRecordUsage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const err = error as any;
      const msg = err.response?.data?.message || t('record_usage_error');
      toast.error(Array.isArray(msg) ? msg[0] : msg);
    }
  };
  ...
  const executeDelete = async () => {
    if (deleteConfirmId) {
      try {
        ...
        const err = error as any;
        const msg = err.response?.data?.message || t('delete_chemical_failed');
        toast.error(Array.isArray(msg) ? msg[0] : msg);
      }
    }
  };
  ...
  const stats = useMemo(() => {
    return {
      ...
      totalStock: chemicals.reduce((acc, c) => acc + c.quantity_stock, 0),
      lowStock: chemicals.filter((c) => c.quantity_stock <= 5).length,
      expiring: expiringAlerts.length,
    };
  }, [chemicals, expiringAlerts]);
    ...
    <div className="max-w-[1200px] w-full mx-auto animate-in fade-in duration-300 h-full flex flex-col space-y-4">
      <div className="flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-2">
          <div>
            ...
            <p className="text-[13px] text-[#64748B] dark:text-slate-400 mt-1">
              Quản lý danh mục hóa chất, số lượng tồn kho và theo dõi hạn sử dụng.
            </p>
          </div>
          <button
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <StatMini
            label="Tổng tồn kho"
            value={Math.round(stats.totalStock * 10) / 10}
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm dark:shadow-slate-900/50 border border-[#E0E0E0] dark:border-slate-800 flex flex-col flex-1 min-h-0 overflow-hidden">
        ...
        <table className="w-full text-left border-collapse">
          <thead>
            <tr className="border-b border-[#E0E0E0]/50 dark:border-slate-800/50 bg-slate-50/50 dark:bg-slate-800/30 backdrop-blur-sm sticky top-0 z-10">
              ...
              <th className="px-6 py-4 text-[13px] font-semibold text-[#757575] dark:text-slate-400 w-[15%] text-right">
                {t('stock_quantity')}
              </th>
              <th className="px-6 py-4 text-[13px] font-semibold text-[#757575] dark:text-slate-400 w-[15%] text-center">
                {t('unit')}
              </th>
              <th className="px-6 py-4 text-[13px] font-semibold text-[#757575] dark:text-slate-400 text-left">
                {t('expiration_date')}
              </th>
              <th className="px-6 py-4 text-[13px] font-semibold text-[#757575] dark:text-slate-400 text-left">
                Bảo quản
          ...
          <tbody className="divide-y divide-[#E0E0E0] dark:divide-slate-800">
              ...
              Array.from({ length: 4 }).map((_, i) => (
                <tr key={`skel-${i}`} className="animate-pulse bg-white dark:bg-slate-900">
                  <td className="px-6 py-4">
                    <div className="h-4 bg-[#E0E0E0] rounded w-32"></div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="h-4 bg-[#E0E0E0] rounded w-20"></div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="h-4 bg-[#E0E0E0] rounded w-16 ml-auto"></div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="h-4 bg-[#E0E0E0] rounded w-12 mx-auto"></div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="h-4 bg-[#E0E0E0] rounded w-24"></div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="h-6 bg-[#E0E0E0] rounded w-20 ml-auto"></div>
                  </td>
                </tr>
              ))}
            ...
            {!isLoading && filteredChemicals.length === 0 ? (
              <tr>
                <td colSpan={6} className="py-12 text-center text-[#757575] dark:text-slate-400">
                  ...
                  <button
                    ...
                    className="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-xl font-bold text-[14px] transition-all duration-300 shadow-lg shadow-blue-500/30 hover:-translate-y-0.5"
                  >
                    {t('add_chemical_now')}
                  </button>
                </td>
              ...
              filteredChemicals.map((c) => {
                const isLowStock = c.quantity_stock <= ((c as any).min_stock_alert ?? 5);
                return (
                  <tr
                    key={c.id}
                  ...
                  >
                    ...
                    <td className="px-6 py-4 text-[14px] text-right">
                      <span
                        className={`font-semibold ${isLowStock ? 'text-red-500' : 'text-[#212121] dark:text-slate-100'}`}
                      >
                        {c.quantity_stock}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-[14px] text-center text-[#757575] dark:text-slate-400">
                      {c.unit}
                    </td>
                    <td className="px-6 py-4 text-[14px] text-[#757575] dark:text-slate-400">
                      {c.expiration_date
                        ? new Date(c.expiration_date).toLocaleDateString('vi-VN')
                        : '-'}
                    </td>
                    <td className="px-6 py-4 text-[14px] text-[#757575] dark:text-slate-400">
                      {(c as any).storage_conditions || '-'}
                    </td>
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-1">
                        <button
                          onClick={() => handleOpenUsageModal(c)}
                          className="p-1.5 text-[#757575] dark:text-slate-400 hover:text-[#2E7D32] hover:bg-[#E8F5E9] dark:bg-green-900/30 rounded transition-colors"
          ...
          <div className="bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl rounded-2xl shadow-2xl dark:shadow-slate-900/50 w-full max-w-md overflow-hidden border border-white/20 dark:border-slate-700/50 animate-in zoom-in-95 duration-200">
            <div className="p-4 border-b border-[#E0E0E0]/50 dark:border-slate-800/50 flex justify-between items-center bg-slate-50/50 dark:bg-slate-800/30">
              <h3 className="font-bold text-[#212121] dark:text-slate-100 text-[16px]">
                {isEditing ? t('edit_chemical_info') : t('add_new_chemical')}
              </h3>
              <button
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                <label className="block text-[13px] font-medium text-[#757575] dark:text-slate-400 mb-1">
                  {t('chemical_name')} <span className="text-red-500">*</span>
                </label>
                <input
                  required
                  type="text"
              ...
              <div>
                <label className="block text-[13px] font-medium text-[#757575] dark:text-slate-400 mb-1">
                  {t('chemical_formula_label')}
                </label>
                <input
                  type="text"
                  value={formData.formula}
                  onChange={(e) => setFormData({ ...formData, formula: e.target.value })}
                  className="w-full px-3 py-2 border border-[#E0E0E0] dark:border-slate-800 rounded-md text-[14px]"
              ...
              <div className="flex gap-4">
                <div className="flex-1">
                  <label className="block text-[13px] font-medium text-[#757575] dark:text-slate-400 mb-1">
                    {t('quantity_stock_label')} <span className="text-red-500">*</span>
                  </label>
                  <input
                    required
                    type="number"
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, quantity_stock: Number(e.target.value) })
                    }
                    className="w-full px-3 py-2 border border-[#E0E0E0] dark:border-slate-800 rounded-md text-[14px]"
                  />
                </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    min="0"
                    value={(formData as any).min_stock_alert}
                    onChange={(e) => setFormData({ ...formData, min_stock_alert: Number(e.target.value) } as any)}
                    className="w-full px-3 py-2 border border-[#E0E0E0] dark:border-slate-800 rounded-md text-[14px]"
                  />
                </div>
              </div>
              <div>
                ...
                <input
                  type="text"
                  value={(formData as any).storage_conditions || ''}
                  onChange={(e) => setFormData({ ...formData, storage_conditions: e.target.value } as any)}
                  className="w-full px-3 py-2 border border-[#E0E0E0] dark:border-slate-800 rounded-md text-[14px]"
                  placeholder="Vd: Tủ lạnh 4°C, Tránh ánh sáng..."
                />
              </div>
              <div className="pt-4 flex justify-end gap-3 border-t border-[#E0E0E0]/50 dark:border-slate-800/50 mt-6">
                <button
                  type="button"
                  onClick={() => setIsModalOpen(false)}
                  className="px-4 py-2 text-[#757575] dark:text-slate-400 hover:bg-[#F5F5F5] dark:hover:bg-slate-800 rounded-lg transition-colors text-[14px] font-medium"
                >
                  {t('cancel')}
                </button>
                <button
                  type="submit"
                  className="px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white rounded-lg font-bold transition-all duration-300 shadow-lg shadow-blue-500/30 hover:shadow-blue-500/50 hover:-translate-y-0.5 text-[14px]"
                >
                  {isEditing ? t('update') : t('add_new_btn')}
                </button>
              </div>
            </form>
          </div>
            ...
            <form onSubmit={handleRecordUsage} className="p-6 space-y-4">
              <div>
                ...
                <input
                  required
                  type="number"
                  value={usageData.booking_id}
                  onChange={(e) =>
                    setUsageData({ ...usageData, booking_id: Number(e.target.value) })
                  }
                  className="w-full px-3 py-2 border border-[#E0E0E0] dark:border-slate-800 rounded-md text-[14px]"
                />
              </div>
              <div>
                <label className="block text-[13px] font-medium text-[#757575] dark:text-slate-400 mb-1">
                  {t('usage_quantity_label')} <span className="text-red-500">*</span>
                </label>
                <input
                  required
                  type="number"
                  ...
                  onChange={(e) =>
                    setUsageData({ ...usageData, quantity_used: Number(e.target.value) })
                  }
                  className="w-full px-3 py-2 border border-[#E0E0E0] dark:border-slate-800 rounded-md text-[14px]"
                />
              </div>
              <div className="pt-4 flex justify-end gap-3 border-t border-[#E0E0E0]/50 dark:border-slate-800/50 mt-6">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white rounded-lg font-bold transition-all duration-300 shadow-lg shadow-emerald-500/30 hover:shadow-emerald-500/50 hover:-translate-y-0.5 text-[14px]"
                >
                  {t('record_btn')}
                </button>
              </div>
